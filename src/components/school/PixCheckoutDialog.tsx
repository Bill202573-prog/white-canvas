...
export default function PixCheckoutDialog({
  open,
  onOpenChange,
  cobrancaId,
  valor,
  ...
  const generatePixQrCode = async () => {
    ...
    console.log('Generating PIX for cobranca:', cobrancaId);

    try {
      const { data, error: fnError } = await supabase.functions.invoke('generate-pix-qrcode', {
  ...
  const checkPaymentStatus = async () => {
    ...
    try {
      const { data, error: fnError } = await supabase.functions.invoke('check-pix-payment', {
        body: { 
          ...
          {(step === 'qrcode' || step === 'checking') && pixData && (
            <div className="space-y-4">
              ...
              <p className="text-xs text-muted-foreground text-center">
                Após efetuar o pagamento, clique no botão acima para confirmar.
              </p>
            </div>
          )}
            ...
            <div className="flex flex-col items-center justify-center py-8 gap-4">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-muted-foreground">
                  Obrigado por efetuar o pagamento da sua mensalidade.
                </p>
